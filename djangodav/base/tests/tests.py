...
class TestBaseDavResource(TestCase):
    ...
    @patch(
        "djangodav.base.resources.BaseDavResource.get_children", Mock(return_value=[])
    )
    def test_get_descendants(self):
        self.assertEqual(
            list(self.resource.get_descendants(depth=1, include_self=True)),
            [self.resource],
        )
    ...
    def test_move_collection(self):
        ...
        src = MockCollection(
            "/path/to/src/", get_children=Mock(return_value=[child]), delete=Mock()
        )
    ...
    def test_copy_collection(self):
        ...
        dst.create_collection.assert_called_with()
        self.assertEqual(
            child.copy.call_args[0][0].path, ["path", "to", "dst", "child"]
        )
